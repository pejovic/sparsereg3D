...
sparsereg3D.ncv <- function(sparse.reg, lambda, step = FALSE, ols = FALSE, all = FALSE, lambda.1se = FALSE, grouped = TRUE, w = NULL, alpha = 1){
  ...
  return(out)
    ...
    for(i in 1:length(profile.fold.list)){
      ...
      # Inner crossvalidation partitioning
      tmp <- stratfold3d(target.name = target.name[2], other.names = kmean.vars , data = training.data, num.folds = num.folds, seed = seed, num.means = num.means, cum.prop = cum.prop)
      inner.profile.fold.list <- tmp$profile.fold.list
      inner.obs.fold.list <- tmp$obs.fold.list
      ...
      models.ncv[[i]] <- as.list(c(coefficients = coef.list, lambda = lambda.min, weight = weight))
      # Prediction on test set
      test.pred <- predict(lasso, s = lambda.min, newx = as.matrix(test.data[,-c(1:3)]), type = "response")[,,1]
      #test.pred <- pmax(test.pred, target.min/3)

      training.pred <- predict(lasso, s = lambda.min, newx = as.matrix(training.data[,-c(1:3)]), type = "response")[,,1]
      #training.pred <- pmax(training.pred, target.min/3)

      # Assembling predictions for final model assessment
      test.obs.pred <- data.frame(obs = test.data[,target.name], pred = as.data.frame(test.pred))
      test.prediction <- rbind(test.prediction,test.obs.pred)


      training.obs.pred <- data.frame(obs = training.data[,target.name], pred = as.data.frame(training.pred))

      obs.pred <- data.frame(test.data, pred = as.matrix(test.pred))
      data.prediction <- rbind(data.prediction, obs.pred)
    }
    test.prediction[,c(4:6)] <- test.prediction[,c(4:6)]*100
    res.list <- as.list(rep(NA,3))
    for(i in 1:length(res.list)){
      res.list[[i]] <- data.frame(obs = test.prediction[, i], pred = test.prediction[, i+3])
    }

    res.list = lapply(res.list, function(x) defaultSummary(x))
    names(res.list) <- target.name

    # Storing results
    if(all){
      out <- list(ncv.results = res.list, accuracy = accuracy, data.prediction = data.prediction, models = models.ncv)
    }else{
      out <- list(ncv.results = res.list)
    }
    return(out)
