...
sparsereg3D.ncv <- function(sparse.reg, lambda, seed = 321){
  ...
  for(i in 1:length(profile.fold.list)){
    ...
    if(!use.hier){
      ...
      final.hier.model <- hierNet(training.main.effects, training.target, zz = training.int.effects, diagonal=FALSE, strong=TRUE, lam = hier.lasso.cv$lamhat, center = FALSE, stand.main = FALSE, stand.int = FALSE)
